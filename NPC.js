...
class NPC {
  ...
  initRifleDirection() {
    ...
    this.rifleDirection.idle = new THREE.Quaternion(
      -0.044,
      -0.061,
      0.865,
      0.495,
    );
    this.rifleDirection.firing = new THREE.Quaternion(
      -0.147,
      -0.04,
      0.784,
      0.6,
    );
    this.rifleDirection.walking = new THREE.Quaternion(
      0.046,
      -0.017,
      0.699,
      0.712,
    );
    this.rifleDirection.shot = new THREE.Quaternion(
      -0.133,
      -0.144,
      -0.635,
      0.747,
  ...
  shoot() {
    if (this.bulletHandler === undefined)
      this.bulletHandler = this.app.bulletHandler;
  ...
  update(dt) {
    ...
    if (this.calculatedPath && this.calculatedPath.length) {
      ...
      if (!this.dead && this.waypoints !== undefined && !this.aggro)
        this.newPath(this.randomWaypoint);
    }
  }
}
